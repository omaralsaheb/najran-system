      ...
      : `<div class="daily-rail">${open.map((task) => {
          ...
          return `<article class="daily-card prio-${esc(task.priority || 'mid')} ${late ? 'late' : ''}">
            <div class="daily-card-top">
              <span class="daily-time">${Number.isNaN(due.getTime()) ? '—' : due.toLocaleTimeString(getDateLocale(), { hour: '2-digit', minute: '2-digit' }).replace(BIDI_RE, '')}</span>
...
function weekView(tasks, selectedDate) {
  ...
  const dayHeads = days.map((entries, index) => {
    ...
    return `<button class="wg-day ${key === todayKey ? 'today' : ''} ${key === selectedDate ? 'picked' : ''}" data-action="set-task-date" data-date="${key}">
      <small>${date.toLocaleDateString(getDateLocale(), { weekday: 'short' }).replace(BIDI_RE, '')}</small>
  ...
  for (let hour = startHour; hour < endHour; hour += 1) {
    const label = new Date(2000, 0, 1, hour).toLocaleTimeString(getDateLocale(), { hour: 'numeric' }).replace(BIDI_RE, '');
      ...
      return `<div class="wg-cell ${key === todayKey ? 'today' : ''}">
        ${shown.map(({ task, due }) => `
          <div class="wg-task prio-${esc(task.priority || 'mid')} ${task.status === 'done' ? 'is-done' : ''}"
            ...
            <span class="wg-time">${due.toLocaleTimeString(getDateLocale(), { hour: '2-digit', minute: '2-digit' }).replace(BIDI_RE, '')}</span>
  ...
  return `
    <section class="week-board">
      <header class="week-head">
        ...
        <strong>${start.toLocaleDateString(getDateLocale(), { day: 'numeric', month: 'long' }).replace(BIDI_RE, '')} — ${new Date(start.getTime() + 6 * DAY_MS).toLocaleDateString(getDateLocale(), { day: 'numeric', month: 'long', year: 'numeric' }).replace(BIDI_RE, '')}</strong>
      </header>
...
export function renderTasks() {
  ...
  const selectedLabel = selectedDate === 'all'
    ? 'كل الأيام'
    : dateFromKey(selectedDate).toLocaleDateString(getDateLocale(), { weekday: 'long', day: 'numeric', month: 'long' }).replace(BIDI_RE, '');
    ...
    <section class="task-date-navigator">
      ...
      <div class="task-date-rail">
        ${railDates.map((key) => {
          ...
          return `<button class="task-date-day ${selectedDate === key ? 'active' : ''} ${isToday ? 'today' : ''}" data-action="set-task-date" data-date="${key}"><span>${date.toLocaleDateString(getDateLocale(), { weekday: 'short' }).replace(BIDI_RE, '')}</span><strong>${date.getDate()}</strong><small>${count ? `${count} مهام` : 'فارغ'}</small></button>`;
        }).join('')}
      </div>
    </section>
...
function calendarKeyLabel(key) {
  ...
  return dateFromKey(key).toLocaleDateString(getDateLocale(), { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(BIDI_RE, '');
}
...
function renderCalendarWorkspace() {
  ...
  const weekdays = Array.from({ length: 7 }, (_, index) => new Date(2026, 1, 1 + index)
    .toLocaleDateString(getDateLocale(), { weekday: 'short' }).replace(BIDI_RE, ''));
      ...
      <div class="calendar-workspace-grid">
        <article class="calendar-month-panel">
          <div class="calendar-month-head">
            <div><span class="section-kicker">الجدول الشهري</span><h2>${calendarCursor.toLocaleDateString(getDateLocale(), { month: 'long', year: 'numeric' }).replace(BIDI_RE, '')}</h2></div>
        ...
        <aside class="calendar-day-agenda">
          <div class="calendar-selected-date"><span>${selectedDate.getDate()}</span><div><small>${selectedDate.toLocaleDateString(getDateLocale(), { weekday: 'long' }).replace(BIDI_RE, '')}</small><strong>${selectedDate.toLocaleDateString(getDateLocale(), { month: 'long', year: 'numeric' }).replace(BIDI_RE, '')}</strong></div><b>${selectedEntries.length}</b></div>
