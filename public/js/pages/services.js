    ...
    <div class="services-layout">
      ...
      <aside class="services-side">
        <div class="attendance-card">
          <div class="attendance-head"><span class="service-icon green"><i class="fi fi-rr-fingerprint"></i></span><div><strong>دوام اليوم</strong><small>${new Date().toLocaleDateString(getDateLocale(), { weekday: 'long', day: 'numeric', month: 'long' }).replace(BIDI_RE, '')}</small></div></div>
          <div class="attendance-times"><div><small>وقت الدخول</small><strong>${today.checkIn ? new Date(today.checkIn).toLocaleTimeString(getDateLocale(), { hour: '2-digit', minute: '2-digit' }).replace(BIDI_RE, '') : '—'}</strong></div><div><small>وقت الخروج</small><strong>${today.checkOut ? new Date(today.checkOut).toLocaleTimeString(getDateLocale(), { hour: '2-digit', minute: '2-digit' }).replace(BIDI_RE, '') : '—'}</strong></div></div>
