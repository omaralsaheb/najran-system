...
function renderChat() {
  ...
  const contacts = state.employees.filter((e) => e.id !== state.currentUser.id);
  const active = contacts.find((e) => e.id === state.activeChatUser);
  ...
  render(`
    ...
    <div class="chat-shell">
      <aside class="chat-sidebar">
        ...
        ${state.chatTab === 'private' ? `<div class="contact-list"><div class="contact-title">رسائل الفريق</div>${contacts.length ? contacts.map((e) => `<button class="contact ${e.id === state.activeChatUser ? 'active' : ''}" data-action="select-chat-user" data-id="${esc(e.id)}"><span class="contact-avatar">${esc((e.name || '؟')[0])}</span><span><strong>${esc(e.name)}</strong><small>${esc(e.roleLabel || '')}</small></span><i class="fi fi-rr-angle-small-left"></i></button>`).join('') : `<div class="soft-empty small"><span>لا توجد محادثات</span></div>`}</div>` : `<div class="chat-info"><i class="fi ${state.chatTab === 'announcements' ? 'fi-rr-megaphone' : 'fi-rr-comments'}"></i><strong>${state.chatTab === 'announcements' ? 'إعلانات الشركة' : 'الدردشة العامة'}</strong><p>${state.chatTab === 'announcements' ? 'قرارات وأخبار رسمية من الإدارة لكل الموظفين.' : 'مساحة مشتركة لكل أعضاء الفريق.'}</p></div>`}
      </aside>
...
export const actions = {
  'chat-tab': (el) => { state.chatTab = el.dataset.tab; renderChat(); },
