...
import { getLanguage, getLocale, t, translateDOM, getDateLocale, BIDI_RE } from '../i18n.js';
...
function lastSevenLabels() {
  ...
  return Array.from({ length: 7 }, (_, index) => {
    ...
    return day.toLocaleDateString(getDateLocale(), { weekday: 'short' }).replace(BIDI_RE, '');
  });
}
  ...
  render(`
    ...
    <div class="home-grid">
      ...
      <section class="home-card home-calendar-card">
        <div class="home-card-head"><div><span class="section-kicker">${now.toLocaleDateString(getDateLocale(), { month: 'long', year: 'numeric' }).replace(BIDI_RE, '')}</span><h2>التقويم</h2></div><button data-action="go" data-page="calendar">فتح التقويم <i class="fi fi-rr-arrow-small-left"></i></button></div>
        <div class="home-calendar">${calendarMarkup(mine)}</div>
        <div class="home-agenda">${upcoming.length ? upcoming.map((task) => `<div><span>${new Date(task.deadline).toLocaleDateString(getDateLocale(), { day: '2-digit', month: 'short' }).replace(BIDI_RE, '')}</span><strong>${esc(task.title)}</strong></div>`).join('') : `<small>لا توجد مواعيد قادمة</small>`}</div>
      </section>
