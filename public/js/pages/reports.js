...
function timeText(timestamp) {
  if (!timestamp) return '—';
  return new Date(timestamp).toLocaleTimeString(getDateLocale(), { hour: '2-digit', minute: '2-digit' }).replace(BIDI_RE, '');
}
...
export function renderReports() {
  ...
  const rangeLabel = `${model.range.start.toLocaleDateString(getDateLocale(), { day: 'numeric', month: 'short' }).replace(BIDI_RE, '')} — ${model.range.end.toLocaleDateString(getDateLocale(), { day: 'numeric', month: 'short', year: 'numeric' }).replace(BIDI_RE, '')}`;
  ...
  render(`
    <section class="operations-report-page">
      <header class="company-print-letterhead">
        ...
        <div class="company-letterhead-copy"><span>NAJRAN AGENCY</span><h1>تقرير الحضور وأداء الفريق</h1><p>${model.period === 'week' ? 'تقرير أسبوعي' : 'تقرير شهري'} · ${esc(rangeLabel)} · تاريخ الإصدار ${esc(new Date().toLocaleDateString(getDateLocale(), { day: '2-digit', month: 'long', year: 'numeric' }).replace(BIDI_RE, ''))}</p></div>
      </header>
